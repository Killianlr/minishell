...
/*   Updated: 2023/12/21 15:02:24 by fserpe           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!ms_strjoin_size(buf, get_var_env(pars, pars->i), data->size))
		return (NULL);
	data->y = ft_strlen(buf);
	if (is_var_env(pars->av[pars->i]))
	{
		while (pars->av[pars->i])
		{
			if (is_whitespace(pars->av[pars->i]) || is_sep(pars->av[pars->i])
...
char	*copy_str(t_pars *pars)
{
	...
	if (!data)
		return (NULL);
	data->size = size_for_line(pars) + 1;
